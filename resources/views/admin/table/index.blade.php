...
<div class="col-xl-12 col-lg-12 col-sm-12  layout-spacing">
    ...
    <div class="widget-content widget-content-area br-8">
        <table id="tables-table" class="table dt-table-hover" style="width:100%">
            <thead>
                <tr>
                    <th width="7%">No</th>
                    <th>Code</th>
...
<script>
    $(document).ready(function() {
        ...
        $('#tables-table').DataTable({
            ...
            serverSide:true,
            ajax: {
            url: "{{ route('tables.get-data') }}",
                error: function(xhr, textStatus, errorThrown) {
                    $('#tables-table').DataTable().clear().draw(); // Bersihkan tabel
        ...
        $("div.toolbar").html('<button class="ms-2 btn btn-primary tables-add" type="button" data-bs-target="#tabs-add-table">'+
                                '<span>Create Table</span>'+
                                '<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-plus"><line x1="12" y1="5" x2="12" y2="19"></line><line x1="5" y1="12" x2="19" y2="12"></line></svg>'+
                            '</button>');

        // Event create by Modal
        $(document).on('click', '.tables-add', function() {
            var getTarget = $(this).data('bs-target');

            $.get("{{ route('tables.modal-add') }}", function(data) {
        ...
        // Event Edit by Modal
        $(document).on('click', '.tables-edit-table', function() {
            var tableId = $(this).data('bs-target');
            var parts = tableId.split("-");
            var id = parseInt(parts[1]);

            $.get("{{ url('tables/modal-edit') }}/" + id, function(data) {
                $('#modalContainer').html(data);
                $(`${tableId}`).modal('show');
            });
        });
        ...
        $(document).on('click', '.tables-delete-table', function() {
            var tableId = $(this).data('bs-target');
            var parts = tableId.split("-");
            var id = parseInt(parts[1]);

            $.get("{{ url('tables/modal-delete') }}/" + id, function(data) {
                $('#modalContainer').html(data);
                $(`${tableId}`).modal('show');
            });
        });

            ...
            <div class="modal-body p-0 m-0">
                <div class="row my-3 px-3">
                            ...
                            <div class="row justify-content-between align-items-center">
                                ...
                                <div class="col-12 col-md-6">
                                    <div class="w-100 d-flex justify-content-center">
                                        <button class="btn btn-default text-white" id="btn-min" style="background: #0c0f1d !important; padding:5px 10px !important;border-color:#000;opacity:0.8;" {{ (($product->current_stock ?? 0) <= 0) ? 'disabled' : '' }}>-</button>
                                        ...
                                        <button class="btn btn-default text-white" id="btn-add" style="background: #0c0f1d !important; padding:5px 10px !important;border-color:#000;opacity:0.8;" {{ (($product->current_stock ?? 0) <= 0) ? 'disabled' : '' }}>+</button>
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="col-12 px-0 mt-3 addon-section">
                        @forelse ($addons as $addon)
                            <ul class="list-group mb-2">
                                <li class="list-group-item parent-addons rounded-0" style="background-color: #e8e8e8;" data-parent-id="{{ $addon['addon']->id }}" data-status-optional="{{ $addon['addon']->status_optional }}" data-choose="{{ $addon['addon']->choose }}" data-name="{{ $addon['addon']->name }}">
                                    {{ $addon['addon']->name }}
                                    <span>
                                ...
                                @if (!empty($addon['children']))
                                    <ul class="list-group ml-4">
                                        @foreach ($addon['children'] as $child)
                                            <li class="list-group-item child-addons d-flex justify-content-between align-items-center rounded-0">
                    ...
                    <div class="col-12 d-flex bd-highlight mt-3">
                        <button type="button" class="btn btn-default {{ (($product->current_stock ?? 0) <= 0) ? 'bg-silver' : 'bg-primary' }} text-white w-100" {{ (($product->current_stock ?? 0) <= 0) ? 'disabled' : '' }} id="addToCartButton" data-route="{{ route('mobile.add-item') }}" data-token="{{ csrf_token() }}">Add to Cart</button>
                    </div>
                </div>
            </div>
        </div>
